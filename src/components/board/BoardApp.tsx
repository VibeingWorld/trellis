...
type Dialog = { kind: "workspace" } | { kind: "workspaceSettings" } | { kind: "board" } | { kind: "column" } | { kind: "columnSettings"; column: Column } | { kind: "background" } | { kind: "boardSettings" } | {kind:"account"};
type BoardSettingsDialog = Exclude<Dialog,{kind:"account"}>;
...
export default function BoardApp() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-shell">
      ...
      <div className="board-and-tray">
        <section className={`board-surface ${board?.background === "midnight" || board?.background === "#253b42" ? "dark-board" : ""}`} style={boardStyle(board?.background)}>
          {board ? <>
            <div className="board-header"><div><div className="board-eyebrow"><span className="board-tiny-dot" />A LITTLE ROOM FOR BIG IDEAS</div><div className="board-title-row"><h1>{board.name}</h1>{can("manageWorkspace")&&<button className="icon-button board-menu" aria-label="Board settings" onClick={() => setDialog({ kind: "boardSettings" })}><Icon name="more" /></button>}</div><p>{board.description || "Make a little progress. Move the good things forward."}</p></div>{can("manageWorkspace")&&<button className="button background-button" onClick={() => setDialog({ kind: "background" })}><Icon name="image" size={16} /><span>Background</span></button>}</div>
    ...
    {dialog && dialog.kind!=="account" && <BoardDialog dialog={dialog} board={board} workspaces={state.workspaces} workspace={workspace} workspaceId={activeWorkspaceId} busy={busy} onClose={() => setDialog(null)} onSubmit={async (name, payload, message) => {
...
function BoardDialog({ dialog, board, workspace, workspaces, workspaceId, busy, onClose, onSubmit }: { dialog: BoardSettingsDialog; board?: Board; workspace?:AppState["workspaces"][number]; workspaces:AppState["workspaces"]; workspaceId: string; busy: boolean; onClose: () => void; onSubmit: (name: string, payload: Record<string, unknown>, message: string) => Promise<boolean> }) {
  ...
  const subtitles = { workspace: "A home for related boards, cards, and ideas.", workspaceSettings:"Update how this workspace appears to everyone.", board: "Every good idea starts with a little space.", column: "Give the next step in your workflow a name.", columnSettings: "Keep your work moving at a comfortable pace.", background: "Find a background that feels like your space.", boardSettings: "Rename this board or move it to another workspace." };
...
function AccountDialog({state,busy,onClose,onSubmit}:{state:AppState;busy:boolean;onClose:()=>void;onSubmit:(name:string,payload:Record<string,unknown>,message:string)=>Promise<boolean>}){
  ...
  return <Modal title={isAdmin?"People & permissions":"Your account"} subtitle={isAdmin?"Create accounts and decide exactly what each person can do.":"Your signed-in account details."} onClose={onClose}><div className="account-summary"><span>{state.currentUser?.name?.slice(0,1).toUpperCase()}</span><div><strong>{state.currentUser?.name}</strong><small>{state.currentUser?.email}</small></div><b>{state.currentUser?.role}</b></div>{isAdmin&&<div className="account-admin"><div className="user-list"><button className={!selectedId?"selected":""} onClick={()=>load()}><span>+</span><div><strong>New account</strong><small>Email and password</small></div></button>{state.users?.map(user=><button key={user.id} className={selectedId===user.id?"selected":""} onClick={()=>load(user)}><span>{user.name.slice(0,1).toUpperCase()}</span><div><strong>{user.name}</strong><small>{user.email}</small></div><i className={user.active?"":"inactive"}>{user.role}</i></button>)}</div><form className="user-editor" onSubmit={save}><h3>{selected?`Edit ${selected.name}`:"Create an account"}</h3><div className="user-fields"><label className="field-label">Name<input required maxLength={100} value={name} onChange={event=>setName(event.target.value)}/></label><label className="field-label">Email<input required type="email" maxLength={255} value={email} onChange={event=>setEmail(event.target.value)}/></label></div><div className="user-fields"><label className="field-label">Role<select value={role} onChange={event=>setRole(event.target.value as "admin"|"member")}><option value="member">Member</option><option value="admin">Administrator</option></select></label><label className="field-label">{selected?"New password (optional)":"Password"}<input required={!selected} type="password" minLength={password?12:undefined} maxLength={200} value={password} onChange={event=>setPassword(event.target.value)} placeholder={selected?"Leave unchanged":"At least 12 characters"}/></label></div>{selected&&<label className="active-account"><input type="checkbox" checked={active} onChange={event=>setActive(event.target.checked)}/><span>Account active</span></label>}{role==="member"&&<div className="permission-matrix"><h4>Workspace permissions</h4>{state.workspaces.map(workspace=><section key={workspace.id}><div><strong>{workspace.name}</strong><button type="button" onClick={()=>setPermissions(current=>({...current,[workspace.id]:permissionOptions.map(([id])=>id)}))}>Allow all</button></div><div>{permissionOptions.map(([id,label])=><label key={id}><input type="checkbox" checked={(permissions[workspace.id]||[]).includes(id)} onChange={()=>toggle(workspace.id,id)}/><span>{label}</span></label>)}</div></section>)}</div>}<button className="button primary" disabled={busy||!name.trim()||!email.trim()||(!selected&&password.length<12)}>{busy?"Saving…":selected?"Save account":"Create account"}</button></form></div>}<div className="account-footer">{state.authenticationDisabled?<small>Direct administrator access is enabled on this server.</small>:<button className="button secondary" onClick={async()=>{await fetch(appPath('/api/auth/logout'),{method:'POST'});window.location.assign(appPath('/login'));}}>Sign out</button>}</div></Modal>;
}
