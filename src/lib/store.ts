...
export function getState(user?:SessionUser): AppState {
  ...
  const aiRuns=db.select().from(s.aiRuns).all().filter(item=>cardIds.has(item.cardId)).sort((a,b)=>b.createdAt-a.createdAt);
  const base={workspaces,boards,columns:db.select().from(s.columns).all().filter(item=>boardIds.has(item.boardId)),cards,placements,tags:db.select().from(s.tags).all().filter(item=>workspaceIds.has(item.workspaceId)),cardTags:db.select().from(s.cardTags).all().filter(item=>cardIds.has(item.cardId)),links:db.select().from(s.links).all().filter(item=>cardIds.has(item.cardId)),attachments:db.select().from(s.attachments).all().filter(item=>(item.cardId&&cardIds.has(item.cardId))||(item.boardId&&boardIds.has(item.boardId))),tray:db.select().from(s.tray).all().filter(item=>placements.some(p=>p.id===item.placementId)),relations:db.select().from(s.relations).all().filter(item=>cardIds.has(item.cardId)&&cardIds.has(item.relatedCardId)),boardMembers,workspaceIntegrations,githubLinks,aiRuns,authenticationDisabled:isAuthenticationDisabled()};
  if(!user)return base;
