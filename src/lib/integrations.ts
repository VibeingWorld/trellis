...
export function attachCodexJob(id:string,threadId:string){const result=sqlite.prepare("UPDATE ai_runs SET status='running',codex_thread_id=?,updated_at=? WHERE id=? AND status='claimed'").run(clean(threadId,'Codex task ID',200),Date.now(),clean(id,'Job ID',100));if(result.changes!==1)throw new AppError('This claimed job no longer exists.',409);return{ok:true};}
